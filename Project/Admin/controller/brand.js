...
const fs = require("fs");
const brandModel = require("../model/brand.js").Brand;
...
const getRecordsCount = async (res) => {
  const count = await brandModel.getSearchCount("").catch((rejected) => {
    unkownError(res);
  });
  return count;
};
...
const displayRecords = async (req, res) => {
  ...
  const queryCount = await brandModel.getSearchCount(searchVal).catch((rejected) => {
    unkownError(res);
  });
  ...
  await brandModel
    ...
    .then((resolved) => {
      ...
      if (page > pages) {
        res.render("brand.ejs", { brands: {}, searchVal: "", pagination: false, currentPage: 1, pageStart: 1, pageEnd: 0, maxPage: 1 }); //defaults
      } else {
        ...
        res.render("brand.ejs", { brands: resolved, searchVal: searchVal, pagination: pagination, currentPage: page, pageStart: pageStart, pageEnd: pageEnd, maxPage: pages });
      }
    });
};
...
const deleteRecord = async (req, res) => {
  ...
  const response = await brandModel.deleteRecord(id).catch((rejected) => {
    unkownError(res);
  });
...
const updateRecords = async (req, res) => {
  const id = parseInt(req.body.index);
  const logo = req.file == undefined ? "" : req.file.filename;
  ...
  if (logo != "") {
    await brandModel
      ...
      .then((resolved) => {
        fs.unlink(`./public/images/logo/${resolved}`, (error) => {
          console.log(error);
        });
      });
  }
  ...
  await brandModel
    .updateRecord(id, req.body.brandName, logo)
    .catch((rejected) => {
      unkownError(res);
    })
    .then((resolved) => {
...
const getAll = async (res) => {
  const response = await brandModel.getAll().catch((rejected) => {
    unkownError(res);
  });
